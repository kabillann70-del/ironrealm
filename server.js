...
for(let i=0; i<30; i++) spawnResource('tree'); 
for(let i=0; i<30; i++) spawnResource('rock');
            ...
            if (mob.hp <= 0) {
                ...
                mobDef.drops.forEach(drop => {
                    if (Math.random() < drop.chance) {
                        ...
                        liveWorld.loot[lid] = { id: lid, itemId: drop.item, x: mob.x + (Math.random()-0.5), z: mob.z + (Math.random()-0.5) };
                    }
                });
...
db.connect().then(async () => { server.listen(PORT, () => console.log(`🚀 IronRealm Master Server Live`)); });